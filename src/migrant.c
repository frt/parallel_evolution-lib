...
status_t migrant_create(migrant_t **migrant, int var_size)
{
	...
	*migrant = NULL;
	...
	if (new_migrant == NULL) {
		parallel_evolution_log(LOG_PRIORITY_ERR, MODULE_MIGRANT, "I can't allocate memory for the migrant.");
		return FAIL;
	}
	...
	if (new_migrant->var == NULL) {
		free(new_migrant);
		parallel_evolution_log(LOG_PRIORITY_ERR, MODULE_MIGRANT, "I can't allocate memory for the migrant's variables.");
		return FAIL;
	}
	...
	*migrant = new_migrant;
	...
	parallel_evolution_log(LOG_PRIORITY_DEBUG, MODULE_MIGRANT, "Migrant created.");
	return SUCCESS;
}
